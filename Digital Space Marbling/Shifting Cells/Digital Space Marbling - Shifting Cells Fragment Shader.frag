...
{
    return snoise(vec3( uv, time ) )  + 0.5;
}

float f(vec2 uv, float time)
{
   return fbm( vec3( fbm( vec3( fbm(uv, time/2.0) ).xy, time/2.0) ).xy, time/2.0);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float uvFactor = 10.0;
        ...
        // factor that slows down the time
    float animSlowFactor = 31.4;
    
    // Time varying pixel color
    float f2 = f(uv*uvFactor, iTime/animSlowFactor);


    vec3 col = vec3( f(uv*uvFactor, iTime/animSlowFactor+f2*f2*f2) , f(uv*uvFactor, iTime/animSlowFactor+f2*f2 ), f(uv*uvFactor, iTime/animSlowFactor+f2 ) );
    // Output to screen
